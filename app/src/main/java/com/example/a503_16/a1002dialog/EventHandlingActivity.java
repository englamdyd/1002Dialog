...
/*
        //anonymous class를 이용한 처리
        btn.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View view) {
                Toast.makeText(EventHandlingActivity.this, "익명 클래스를 이용한 처리", Toast.LENGTH_LONG).show();
            }
        });
*/
    }
    ...
    //화면을 터치 했을 때 호출되는 메소드
    //event에 터치에 대한 정보(터치가 발생한 좌표와 동작)가 전부 저장되어 있다.
    @Override
    ...
    //키보드 눌렀을 때 호출되는 메소드
    //누른 키보드 값은 Keycode에 저장되서 넘어온다.
    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        //백버튼을 눌렀다면
        if(keyCode == KeyEvent.KEYCODE_BACK){
            if(System.currentTimeMillis() - initBack > 3000){
                Toast.makeText(this, "3초 안에 두번 누르면 종료됩니다.", Toast.LENGTH_LONG).show();
